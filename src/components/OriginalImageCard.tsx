...
const OriginalImageCard: React.FC<OriginalImageCardProps> = ({ image, onView }) => {
  return (
    <Card className="flex flex-col items-center space-y-4 p-5 shadow-lg hover:shadow-xl transition-shadow duration-300 bg-card border border-border rounded-lg w-full max-w-[300px]">
      <p className="text-base font-medium text-foreground text-center break-all px-2">
        {image.filename}
      </p>
      ...
      <Button onClick={() => onView(image)} className="w-full flex items-center justify-center gap-2 text-primary-foreground bg-primary hover:bg-primary/90">
